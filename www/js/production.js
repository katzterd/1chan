...
(function(){
	var x = {
		socket:     new Dklab_Realplexor("https://pipe.1chan.pl/", "1chan_"),
		processors: [],
	...
	x.addPageProcessor("news/add", function() {
		var validatePost = function(callback) {
			...
			$.post(
				"https://"+ location.host +"/news/add/validate/",
				{
		...
		$("#blog_form_preview_button")
				...
				.click(function() {
					validatePost(function()  {
						$.post(
							"https://"+ location.host +"/news/add/preview/",
							{
								"category":   $("#blog_form [name=category]").val(),
								"link":       $("#blog_form [name=link]").val(),
								"title":      $("#blog_form [name=title]").val(),
		...
		$.getJSON("https://"+ location.host +"/news/cat/", function(data, status) {
			$( "input[name=category]" )
					.autocomplete({
						minLength: 0,
						source: data,
		...
		function() {
		    $(window)
		        .bind("keyup", function(e) {
				    ...
				    if ((e.ctrlKey || e.metaKey) && (e.keyCode==39)) {
					    location.href = "https://"+ location.host +"/news/all/new/";
				    }
			...
			$(".js-favorite-button").click(function() {
				...
				$.getJSON(this.href, function(data, status) {
					if (data.favorite == true)
						$(img).attr("src", "https://"+ location.host +"/ico/favorites-true.png");
					else
						$(img).attr("src", "https://"+ location.host +"/ico/favorites-false.png");
				});
				return false;
			});
			...
			x.addPageProcessor(":moderator", function(path, params) {
                        ...
                        if (confirm("Вы уверены, что хотите удалить посты?")) {
					        ...
					        $(".js-post-group:checked").each(function(el, ind) {
    						    ...
    						    setTimeout((function(id, why) {
    						        return function() { $.getJSON("https://"+ location.host +"/mod/hiddenPost/"+ id +"/?why="+ encodeURIComponent(why)); };
    						    })(id, why), 1000 * ind);
    						});
					    }
					    return false;
				...
				$("#mod_category").click(function() {
					...
					$.getJSON("https://"+ location.host +"/mod/categoryPost/"+ current_id +"/?cat="+ encodeURIComponent(cat));
					$(".b-mod-toolbar").addClass("g-hidden");
					current_id = null;
					return false;
				});
				$("#mod_pinned").click(function() {
					$.getJSON("https://"+ location.host +"/mod/pinnedPost/"+ current_id +"/");
					$(".b-mod-toolbar").addClass("g-hidden");
					current_id = null;
					return false;
				});
				$("#mod_rated").click(function() {
					$.getJSON("https://"+ location.host +"/mod/ratedPost/"+ current_id +"/");
					$(".b-mod-toolbar").addClass("g-hidden");
					current_id = null;
					return false;
				});
				$("#mod_rateable").click(function() {
					$.getJSON("https://"+ location.host +"/mod/rateablePost/"+ current_id +"/");
					$(".b-mod-toolbar").addClass("g-hidden");
					current_id = null;
					return false;
				});
				$("#mod_closed").click(function() {
					$.getJSON("https://"+ location.host +"/mod/closedPost/"+ current_id +"/");
					$(".b-mod-toolbar").addClass("g-hidden");
					current_id = null;
					return false;
				});
				if (params == false) {
				    $("#mod_remove").click(function() {
					    if (confirm("Вы уверены, что хотите скрыть пост "+ current_id +"?")) {
					        var why = prompt("Введите причину модерации:", "мусор (читайте правила)");
						    $.getJSON("https://"+ location.host +"/mod/hiddenPost/"+ current_id +"/?why="+ encodeURIComponent(why));
						    $(".b-mod-toolbar").addClass("g-hidden");
						    current_id = null;
					    }
					    return false;
				...
				$(".b-blog-entry_b-info").bind("dblclick", function() {
					...
					$.getJSON("https://"+ location.host +"/mod/hiddenPost/"+ current_id +"/?why="+ encodeURIComponent("Мусор (читайте правила)"));
				});
				$(".js-moderator-button").removeClass("g-hidden").click(function(e) {
					current_id = $("img", this).attr("alt");
					$.getJSON("https://"+ location.host +"/mod/getPost/"+ current_id +"/" , function(data, status) {
	...
	x.addPageProcessor(/^news\/res\/(\d+)/, function(match) {
		var id = match[1],
			...
			$.post(
				"https://"+ location.host +"/news/res/"+ id +"/add_comment/",
				{
...
*/
						if (result.isValid != true) {
							...
							$.get("https://"+ location.host +"/news/res/"+ id +"/");
		...
		var writing, waiting, writingTimeout = null, _title = document.title,
		   statusReading = function() {
			$.getJSON("https://"+ location.host +"/news/res/"+ id +"/stats/?writing=0");
			writing = false;
		};
			...
			$(".js-cross-link", node).mouseover(function(e) {
				if (!$(this).data("preview_open"))
				{
					...
					if (el.length != 0) {
						...
						var link_ = this;
						$.getJSON("https://"+ location.host +"/news/last_comments/", {id: id}, function(data, status) {
							if (status != "error" && data != false) {
								...
								if (data.post_preview) tip.addClass("m-post-preview").find(".js-comment-id").html('<a href="https://'+ location.host +'/news/res/'+ data.post_id +'/">'+ data.post_title +'</a> (<em>открывающий пост</em>)')
								else tip.find(".js-comment-id").prepend('<a href="https://'+ location.host +'/news/res/'+ data.post_id +'/">'+ data.post_title +'</a> ');
		...
		$("#comment_form_text")
			.bind("keyup", function(e) {
				if (!writing) {
					$.getJSON("https://"+ location.host +"/news/res/"+ id +"/stats/?writing=1");
					writing = true;
				}
		...
		x.subscribe("post_"+ id, "add_post_comment", function(data) {
			    ...
			    commentPreview(node);
			    $.getJSON("https://"+ location.host +"/news/res/"+ id +"/");
		});
		...
		x.addPageProcessor(":moderator", function(path, params) {
		    if (params == false)  {
			    $(".js-remove-button").removeClass("g-hidden").click(function() {
				    ...
				    $.getJSON("https://"+ location.host +"/mod/removePostComment/"+ comment_id +"/");
				    return false;
			    });
	        }
		});
		x.addPageProcessor(":moderator", function(path, params) {
		    if (params == false)  {
			    $(".js-setusercaptcha-button").removeClass("g-hidden").click(function() {
				    var comment_id = $("img", this).attr("alt");
				    $.getJSON("https://"+ location.host +"/mod/setUserCaptcha/"+ comment_id +"/");
				    return false;
			    });
	        }
		});
	...
	x.addPageProcessor(/service\/last_board_posts\/?(\d+)?/, function(match) {
			...
			$(".js-cross-link", node).mouseover(function(e) {
				if (!$(this).data("preview_open"))
				{
					...
					if (el.length != 0) {
						...
						var link_ = this;
						$.getJSON("https://"+ location.host +"/"+ board +"/get/", {id: id}, function(data, status) {
							if (status != "error" && data != false) {
								var tip = $(template("template_comment", data))
									       .mouseover(function(e) { previewTimeout = clearTimeout(previewTimeout); e.stopPropagation(); })
									       .addClass("m-tip")
		...
		if (!page) {
			x.subscribe("board_global", "add_post_comment", function(data) {
					$.getJSON("https://"+ location.host +"/"+ data.board_id +"/get", {id: data.id}, function(data) {
						var node = $(template("template_comment", data));
						...
						if (modMode) {
							$(".js-remove-button", node).removeClass("g-hidden").click(function() {
								...
								$.getJSON("https://"+ location.host +"/"+ board +"/remove/?id="+ comment_id);
	...
	x.addPageProcessor("fav", function(match) {
		...
		$(".js-postload-link").click(function() {
			...
			{
				$.getJSON("https://"+ location.host +"/"+ board +"/get", {id: postloader[board +"_"+ id]}, function(data) {
					for(var i in data) {
		...
		$(".js-thread-load").click(function() {
			...
			$.getJSON("https://"+ location.host +"/"+ board +"/get", {thread_id: nm}, function(data) {
		...
		$(".js-update-post-button").click(function(e) {
			...
			{
				$.getJSON("https://"+ location.host +"/"+ board +"/get", {id: postloader[updateThread]}, function(data) {
					for(var i in data) {
						var node = $(template("template_comment", data[i]));
			...
			$("#comment_form")
				...
				.attr("action", "https://"+ location.host +"/"+ board +"/createPostAjaxForm/")
				.submit(function() {
					$("#comment_form input[type=submit]").attr("disabled", "disabled");
				});

			var statusReading = function(id) {
				$.getJSON("https://"+ location.host +"/"+ board +"/res/"+ id +"/stats/?writing=0");
				writing = false;
			};
			...
			$("#comment_form_text")
				.bind("keyup", function(e) {
					if (!writing) {
						$.getJSON("https://"+ location.host +"/"+ board +"/res/"+ id +"/stats/?writing=1");
						writing = true;
					}
	...
	x.addPageProcessor(/^(operate|alone)\/?(\d+)?$/, function(match) {
		...
		window['board_callback'] = function(data) {
			...
			location.href = "https://"+ location.host +"/"+ board +"/res/"+ data.id +"/";
			throw void(0);
		};
		...
		$(".js-subscribe-checkbox").bind("change", function() {
			if ($(this).is(":checked")) {
				$.getJSON("https://"+ location.host +"/service/subscribeBoard/"+ board +"/");
				console.log("subscibed");
			} else {
 				$.getJSON("https://"+ location.host +"/service/unsubscribeBoard/"+ board +"/"); 
			}
		});
		...
		$(".js-postload-link").click(function() {
			...
			{
				$.getJSON("https://"+ location.host +"/"+ board +"/get", {id: postloader[nm]}, function(data) {
					for(var i in data) {
						var node = $(template("template_comment", data[i]));
				    	$("#placeholder_comment_"+ board +"_"+ nm).append(node);
				    	node.slideUp(0).slideDown(300);
		...
		x.subscribe("board_"+ board, "add_post_comment", function(data) {
				...
				if (updateMode && data.parent_id == updateThread) {
					$.getJSON("https://"+ location.host +"/"+ board +"/get", {id: data.id}, function(data) {
						var node = $(template("template_comment", data));
		...
		$("#board_form")
			.attr("target", "board_form_iframe")
			.attr("action", "https://"+ location.host +"/"+ board +"/createAjaxForm/")
			.submit(function() {
		...
		$(".js-update-post-button").click(function(e) {
			...
			var statusReading = function(id) {
				$.getJSON("https://"+ location.host +"/"+ board +"/res/"+ updateThread +"/stats/?writing=0");
				writing = false;
			};
			...
			$("#comment_form_text")
				.bind("keyup", function(e) {
					if (!writing) {
						$.getJSON("https://"+ location.host +"/"+ board +"/res/"+ updateThread +"/stats/?writing=1");
						writing = true;
					}
		...
		x.addPageProcessor(":moderator", function(path, params) {
		    if (params) {
			    ...
			    link.click(function() {
				    ...
				    $.getJSON("https://"+ location.host +"/"+ board +"/changeTitle", {title:title, description:description}); 
			    });
		    ...
		    if (params) {
			    $(".js-remove-button")
                    .removeClass("g-hidden").click(function() {
				        ...
				        $.getJSON("https://"+ location.host +"/"+ board +"/remove/?id="+ comment_id + (delall ? "&delall=1" : ""));
				        return false;
			        });
	        }
		});
	...
	x.addPageProcessor("live", function() {
		var resetFileds = function(reset) {
		   ...
		   reloadPage = function() {
			$.getJSON("https://"+ location.host +"/live/", function(data, status) {
				if (status != "error") {
					if (data.length == 0) {
		...
		$.getJSON("https://"+ location.host +"/live/set_filter/", function(data) {filter = data});

		$("#filter_link_form").submit(function() {
			$.getJSON("https://"+ location.host +"/live/set_filter/?"+ $("#filter_link_form").serialize(), function(data) {
				reloadPage();
				filter = data;
			});
			return false;
		...
		$(".js-open-right-panel").click(function() {
            $.getJSON("https://"+ location.host +"/live/linksPanel/", {status: "on"}, function(data, status) {
                location.reload();
            });
            return false;
        });
		...
		x.addPageProcessor(":moderator", function(path, params) {
		    if (params) {
			    $(".js-remove-button").removeClass("g-hidden").click(function() {
				    ...
				    $.getJSON("https://"+ location.host +"/mod/removeOnlineLink/"+ link_id +"/");
				    return false;
			    });
	        }
		});
	...
	x.addPageProcessor("news/last_comments", function() {
		...
		x.subscribe("post_last_comments", "add_post_comment", function(data) {
		    $.getJSON("https://"+ location.host +"/news/res/"+ data.post_id +"/getComment/"+ data.id, {"title" : 1}, function(data) {
			    var node = $(template("template_comment", data));
			    $("#placeholder_comment").prepend(node);
	...
	x.addPageProcessor("chat", function() {
	    $(".js-favchatlist").sortable({
	        ...
	        update: function(event, ui) {
	            ...
	            $.post(
				    "https://"+ location.host +"/chat/favorites/",
				    {
	...
	x.addPageProcessor(/^chat\/[0-9A-z\-]{4,45}/, function(match) {
		...
		if (id) {
	        ...
	        var addInfoMessage = function(message) {
	            ...
	            enterChat = function() {
	                $.getJSON("https://"+ location.host +"/chat/"+ id +"/", {"command": "welcome"}, function(data, status) {
	                    if (status !== "error")
	                    {
	                        addInfoMessage("Добро пожаловать в комнату <b>«"+ data.title +'»</b> (<a href="https://'+ location.host +'/chat/log/'+ id +'/">лог комнаты</a>). <br />Описание: '+ data.description +'<br /><a href="/help/rules/#chat">Правила раздела</a>, <a href="/help/markup/">разметка сообщений</a>.');
	                        if (data.info)
		                    ...
		                    setInterval(function() {
		                        $.getJSON("https://"+ location.host +"/chat/"+ id +"/", {"command": "ping"});
		                    }, 1000 * 30);
	                    }
	                });
	        ...
	        $.getJSON("https://"+ location.host +"/chat/"+ id +"/", {"command": "enter"}, function(data, status) {
	            if (status !== "error")
	            {
	                if (data == null)
	...
	x.addPageProcessor("*", function() {
		var to = null,
			...
			$.getJSON("https://"+ location.host +"/live/?num=12", function(data, status) {
				if (status != "error") {
					if (data.length == 0) {
		...
		$(".b-header-block")
			...
			.mouseenter(function() {
				$.getJSON("https://"+ location.host +"/service/getGlobalStats/", {}, function(data, status) {
		...
		$(".js-close-right-panel").click(function() {
		    $.getJSON("https://"+ location.host +"/live/linksPanel/", {status: "off"}, function(data, status) {
	...
	{
		$.post("https://"+ location.host +"/auth", {key: key, hash: hash || 0}, function(data, status) {
			if (status != "error") {
