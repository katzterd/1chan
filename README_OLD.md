...
* Скрипты инсталляции и выполнения фоновых задач
...
### Настройки socket.io
* `SRV_LOCAL_IP` – Адрес, с которого сервер socket.io будет принимать сигналы. Как правило, всегда это `127.0.0.1`.
...
* `SRV_HOST` и `SRV_PORT` – Адрес или хостнейм, с которого сервер socket.io будет принимать внешние подключения. Должны совпадать с конфигурацией nginx, приведенной ниже:
...
### Цветовые стили и кастомизация CSS
Набор цветовых стилей определяется константой `COLOR_THEMES` в `.env`. Первые два стиля являются дефолтными и применяются в зависимости от стиля в системе, и поэтому их не может быть меньше двух. Вы можете добавлять собственные стили после них, разделяя их символом "`|`". 

Для определения собственных URL картинок маскотов и прочего используются файлы `/www/css/themes/normal.custom.css` и `/www/css/themes/omsk.custom.css`. Примеры файлов приведены в той же директории. Для дополнительных стилей соблюдайте ту же схему (два файла на стиль, имена файлов должны совпадать с указанными в константе `COLOR_THEMES`).
...
Движок поставляется с ботом, позволяющим репостить новости в Telegram-канал. Бот работает в составе сервисного скрипта.
* `TG_ENABLE` – функционал бота будет включен, если `"true"`.
* `TG_BOT_TOKEN` – токен вашего бота, полученный через [@botfather](https://core.telegram.org/bots/tutorial).
* `TG_CHANNEL_ALL` – имя канала, в который будут поститься все новости.
* `TG_CHANNEL_APPROVED` – имя канала, в который будут репоститься зааппрувленные нововсти. Опционально. Пустая строка отключает функцию.
  * Имена каналов в конфиге должны начинаться с "@". 
  * Бот должен иметь права администратора в каналах.
