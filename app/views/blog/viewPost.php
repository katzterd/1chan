				...
				<div class="b-blog-entry<?php if ($post['hidden']): ?> m-hidden<?php endif; ?>" id="post_<?php echo($post['id']); ?>">
					...
					<div class="b-blog-entry_b-info" id="post_<?php echo($post['id']); ?>_info">
						...
						<?php endif; ?>
							...
							<span>|</span>
							<span>
								<a href="/news/res/<?php echo($post['id']); ?>/"  class="g-disabled js-post-id-link">
									№<?php echo($post['id']); ?></a>
							</span>
						...
						<?php if($post['closed']): ?>
							Обсуждение закрыто
						<?php else: ?>
							<a href="/news/res/<?php echo($post['id']); ?>/#comment_form" class="js-add-comment-button">
				...
				<div class="l-comments-wrap">
					...
					<div class="b-comment<?php TemplateHelper::isNewComment($comment) and print(" m-new"); ?>" id="comment_<?php echo($comment['id']); ?>">
						<div class="b-comment_b-info">
							<?php echo(TemplateHelper::date('d M Y @ H:i:s', $comment['created_at'])); ?>, <a href="#<?php echo($comment['id']); ?>">№</a><a href="javascript://" class="js-paste-link" name="<?php echo($comment['id']); ?>"><?php echo($comment['id']); ?></a>
							<a href="#" class="js-remove-button g-hidden">
								<img src="/ico/remove.gif" width="16" height="16" alt="<?php echo($comment['id']); ?>" />
							</a>
					...
					<?php if(!$post['closed']): ?>
					<form action="/news/res/<?php echo($post['id']); ?>/add_comment/" method="post" id="comment_form">
